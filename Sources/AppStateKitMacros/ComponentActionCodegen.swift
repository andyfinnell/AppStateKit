...
struct ComponentActionCodegen {
    static func codegen(from component: Component) -> [DeclSyntax] {
        ...
        let implementations = component.actions.map {
            generateActionImplementation(from: $0)
        }
...
private extension ComponentActionCodegen {
    static func generateActionImplementation(from action: Action) -> DeclSyntax? {
    ...
    static func generateImplementationBody(from action: Action, implementation: AutogeneratedImplementation) -> String {
        switch implementation {
        case let .updateStateProperty(propertyName):
            if let parameterName = implementationParameterName(from: action, at: 0) {
                return "state.\(propertyName) = \(parameterName)"
            } else {
