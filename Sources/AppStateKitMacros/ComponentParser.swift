...
private extension ComponentParser {
    ...
    static func computeActions(from varDecl: VariableDeclSyntax) -> ([Action], [ComponentOutput]) {
        let updatableAttribute = varDecl.attributes.compactMap { attribute -> AttributeSyntax? in
            guard case let .attribute(attr) = attribute,
                let identifierType = attr.attributeName.as(IdentifierTypeSyntax.self),
                  (identifierType.name.text == "Updatable" || identifierType.name.text == "BatchUpdatable") else {
                return nil
            }
            return attr
        }.first
        guard let updatableAttribute, let identifierType = updatableAttribute.attributeName.as(IdentifierTypeSyntax.self) else {
            return ([], [])
        }
        
        let isBatchUpdatable = identifierType.name.text == "BatchUpdatable"
        ...
        for binding in varDecl.bindings {
            ...
            if isBatchUpdatable {
                implementation = AutogeneratedImplementation.batchUpdateStateProperty(
                    identifierPattern.identifier.text,
                    shouldOutputExpr: shouldOutputExpr
                )
                parameterType = batchParameterType(from: typeDecl)
            } else {
                implementation = AutogeneratedImplementation.updateStateProperty(
                    identifierPattern.identifier.text,
                    shouldOutputExpr: shouldOutputExpr
                )
                parameterType = typeDecl
            }
            let action = Action(
                label: actionName,
                parameters: [
                    Parameter(label: nil, type: parameterType)
                ],
                composition: nil,
    ...
    static func batchParameterType(from typeDecl: TypeSyntax) -> TypeSyntax {
        guard let arraySyntax = typeDecl.as(ArrayTypeSyntax.self) else {
            return typeDecl
        }
        let type: TypeSyntax = """
