...
public final class ViewStore<State, Action, Environment>: ObservableObject {
    ...
    public init(store: Store<State, Action, Environment>, removeDuplicatesBy isDuplicate: @escaping (State, State) -> Bool) {
        state = store.state
...
public extension ViewStore where State: Equatable {
    convenience init(store: Store<State, Action, Environment>) {
